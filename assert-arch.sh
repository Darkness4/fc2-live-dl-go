...
find target/ -type f -executable \( -name '*aarch64*' -o -name '*arm64*' -o -name '*x86_64*' -o -name '*amd64*' -o -name '*x86-64*' -o -name '*riscv64*' \) -print0 | while read -d $'\0' file; do
  expected_arch=$(echo "$file" | cut -d ':' -f 2- | grep -o -E 'aarch64|arm64|x86_64|x86-64|amd64|RISC-V|riscv64' | head -1)
  ...
  if [ "$expected_arch" = "$detected_arch" ]; then
    ...
    echo "$file does not match expected architecture: expected '$expected_arch', detected '$detected_arch'"
    exit 1
  fi
done
